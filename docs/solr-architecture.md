...
configset per site. Unity 1 currently declares 16 Solr-hosted sites in
`project/project.yml`; 6 have a committed Drupal `solr_default` server.

Ten hosting declarations (`odscni`, `fiscalcommissionni`, `mahinquiry`, `nifiscalcouncil`, `octf`, `employmenttribunalsni`, `independentreviewofeducation`, `investnireview`, `cscsreviewni`, and `urologyservicesinquiry`) do not currently have a committed Drupal `solr_default` server. Their existing configsets are preserved, but the generator intentionally skips them until that application configuration exists.
