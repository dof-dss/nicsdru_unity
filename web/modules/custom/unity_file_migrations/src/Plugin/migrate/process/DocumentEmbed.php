    ...
    if (!empty($matches)) {
      ...
      foreach ($matches as $match) {
        ...
        if ($lastNode = end($nids)) {
          ...
          // Publication Page link used for replacing embedded links.
          // Currently hardcoded node ID but will be dynamic to
          // prevent ID issues.
          $publications_link = Link::createFromRoute(
        ...
        // Then replace these links to publication link ( or publication
        // page if not exists -- for now ).
        // $value = preg_replace($embed_regex, $publications_link,
        // $value);.
